...
def createEmpregado(db: Session, empregado: schemas.EmpregadoCreate):
    db_empregado = models.Empregado(
        nome_completo=empregado.nome_completo,
        id_departamento=empregado.id_departamento,
    )
    db.add(db_empregado)
    db.commit()
...
def getEmpregado(db: Session, empregado_id: int):
    empregado = (
        db.query(models.Empregado)
        .filter(models.Empregado.id == empregado_id)
        .first()
    )
...
def get_empregados_by_departamento(db: Session, departamento_id: int):
    empregados = (
        db.query(models.Empregado)
        .filter(models.Empregado.id_departamento == departamento_id)
        .all()
    )
...
def createDependente(db: Session, dependente: schemas.DependenteCreate):
    db_dependente = models.Dependente(
        nome=dependente.nome, empregado_id=dependente.id_empregado
    )
...
def getdependentes(db: Session, empregado_id: int):
    return (
        db.query(models.Dependente)
        .filter(models.Dependente.id_empregado == empregado_id)
        .all()
    )
